...
def build_direct_pdf(
    *,
    ...
    if page2:
        ...
        s_dvt = ParagraphStyle("dvt", parent=s_jp, fontSize=8, leading=10, alignment=1)
        dvt_data = [[Paragraph("空港", s_jp), Paragraph("距離", s_jp), Paragraph("GS120kt", s_jp), Paragraph("GS140kt", s_jp)]]
        for r in dvt_rows:
            dvt_data.append(
                [
                    Paragraph(str(r[0]), s_jp),
                    str(r[1]),
                    Paragraph(str(r[2]), s_dvt),
                    Paragraph(str(r[3]), s_dvt),
                ]
            )
        ...
        dvt_tbl = Table(
            dvt_data,
            colWidths=[56 * mm, 18 * mm, 88 * mm, 88 * mm],
            style=TableStyle(
                ts
                + [
                    ("FONT", (0, 0), (-1, -1), jp, 8),

...
_SELECT_OPTION_COLORS: dict[str, str] = {
    "ohibirin": "#a21caf",
    "jcab": "#15803d",
    "general": "#1d4ed8",
    "instructor": "#dc2626",
...
def _select_colorizer_js() -> str:
    colors_json = json.dumps(_SELECT_OPTION_COLORS, ensure_ascii=False)
    return f"""
(function() {{
  ...
  function paintRegistrySplit(el, text) {{
    ...
    if (!tk) return false;
    const html =
      '<span style="color:' + COLORS[tk] + ' !important;font-weight:700">' + m[1] + '</span> ' +
      '<span style="color:#111827 !important;font-weight:500">' + m[2] + '</span>';
  ...
  window.addEventListener("click", () => setTimeout(() => run(doc), 50));
  window.addEventListener("keydown", () => setTimeout(() => run(doc), 50));
}})();
"""
...
def _inject_colored_select_options() -> None:
    ...
    components.html は iframe 内で動くため、スクリプトは親 document へ注入する。
    可能なら st.html でも同じスクリプトを流す。
    """
    js = _select_colorizer_js()
    if hasattr(st, "html"):
        try:
